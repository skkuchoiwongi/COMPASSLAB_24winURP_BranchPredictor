...
class BHT(params: BHTParams)(implicit val p: Parameters) extends HasCoreParameters {
  ...
  def get(addr: UInt): BHTResp = {
    ...
    when ((tagMatch4 === 1.U)) {
      res.value := T4_pred(index4(addr,history))
    }.elsewhen ((tagMatch3 === 1.U)) {
      res.value := T3_pred(index3(addr,history))
    }.elsewhen ((tagMatch2 === 1.U)) {
      res.value := T2_pred(index2(addr,history))
    }.elsewhen ((tagMatch1 === 1.U)) {
      res.value := T1_pred(index1(addr,history))
    }.otherwise (res.value := Mux(resetting, 0.U, table(index(addr,history))))
    ...
    when((tagMatch44===1.U)){ //provider가 T4인 경우
      ...
      when (tagMatch33===1.U) { //alternative가 T3인 경우
        when(T3_pred(index3(addr, d.history))(2) =/= T4_pred(index4(addr, d.history))(2)){ //예측이 서로 다른 경우
          ...
          wen_T1_u := false.B
          waddr_T4 := index4(addr, d.history)
          wdata_T4_u := Mux(mispredict, Mux(u4 === 0.U, 0.U, u4 -1.U),Mux(u4===3.U,3.U,u4+1.U)) //mispredict에 따라 T4의 u 증감함
        }
        ...
        when(T2_pred(index2(addr, d.history))(2) =/= T4_pred(index4(addr, d.history))(2)){
          ...
          wen_T1_u := false.B
          waddr_T4 := index4(addr, d.history)
          wdata_T4_u := Mux(mispredict, Mux(u4 === 0.U, 0.U, u4 -1.U),Mux(u4===3.U,3.U,u4+1.U))
        }
      ...
      when(tagMatch22===1.U) {
        when(T2_pred(index2(addr, d.history))(2) =/= T3_pred(index3(addr, d.history))(2)){
          ...
          wen_T1_u := false.B
          waddr_T3 := index3(addr, d.history)
          wdata_T3_u := Mux(mispredict, Mux(u3 === 0.U, 0.U, u3 -1.U),Mux(u3===3.U,3.U,u3+1.U))
        }
      ...
      when(tagMatch11===1.U) {
        when(T1_pred(index1(addr, d.history))(2) =/= T2_pred(index2(addr, d.history))(2)){
          ...
          wen_T1_u := false.B
          waddr_T2 := index2(addr, d.history)
          wdata_T2_u := Mux(mispredict, Mux(u2 === 0.U, 0.U, u2 -1.U),Mux(u2===3.U,3.U,u2+1.U))
        }
      }.otherwise {
      ...
      when(table(index(addr, d.history))(2) =/= T2_pred(index2(addr, d.history))(2)){
        ...
        wen_T1_u := false.B
        waddr_T1 := index1(addr, d.history)
        wdata_T1_u := Mux(mispredict, Mux(u1 === 0.U, 0.U, u1 -1.U),Mux(u1===3.U,3.U,u1+1.U))
      }
      when(mispredict) {
  ...
  private val T1_tag = Mem(1024,UInt(8.W))
  private val T2_tag = Mem(1024,UInt(8.W))
  private val T3_tag = Mem(1024,UInt(8.W))
  private val T4_tag = Mem(1024,UInt(8.W))
