...
class BHT(params: BHTParams)(implicit val p: Parameters) extends HasCoreParameters {
  def index(addr: UInt) = { //단순 하위 8비트만 사용
    addr(7,0)
  }

  def dot(hist: UInt, weight: Vec[SInt]): SInt = { //내적을 진행하는 함수
    ...
    dot_product + 1.S //마지막에 bias를 더해줌
  }
  def get(addr: UInt): BHTResp = {
    val res = Wire(new BHTResp)
    val dot_result = dot(history,table(index(addr)))
    //mux를 이용하여 내적 크기가threshold보다 크면 양수 음수로 prediction 판별, 작으면 무조건 taken으로 설정
    res.value := Mux(resetting, 0.U, Mux(dot_result(params.historyBits-2,0)>threshold,Mux(dot_result(params.historyBits-1),0.U,1.U),1.U)) 
    res.history := history
    res.dot := dot_result
    res
  }
  def updateTable(addr: UInt, d: BHTResp, taken: Bool): Unit = {
    ...
    when (!resetting) {
      when(d.taken =/= taken||(d.taken===taken)&&(d.dot(params.historyBits-2,0)<threshold)) { //update 조건을 1. misprediction 2. dot product의 크기가 threshold보다 작은 경우로 설정
        wen := true.B
  ...
  private val threshold = 10.U //threshold value
