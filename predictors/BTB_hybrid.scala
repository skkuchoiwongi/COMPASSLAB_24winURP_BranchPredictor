...
// TAGE + 'reset u' + USE_ALT + perceptron version
...
class BHTResp(implicit p: Parameters) extends BtbBundle()(p) {
  ...
  def taken = Mux(meta_value(1) === 0.U, tage_value(2), perceptron_value).asBool //final prediction
  def strongly_taken = Mux(meta_value(1) === 0.U, (tage_value > 4.U).asUInt, perceptron_value).asBool //strongly taken
}
